...
impl UncompressedBlock {
    ...
    pub fn compress_to_chunk(self, headers: &[Header]) -> Result<Chunk> {
        ...
        let block_bounds = IntegerBounds {
            position: Vec2(index.pixel_position.x() as i32, index.pixel_position.y() as i32),
            size: index.pixel_size,
        };
        let expected_byte_size = header.channels.bytes_per_pixel_section(block_bounds);
    ...
    ) -> Vec<u8>
    {
        ...
        let block_bounds = IntegerBounds {
            position: Vec2(block_index.pixel_position.x() as i32, block_index.pixel_position.y() as i32),
            size: block_index.pixel_size,
        };
        let byte_count = channels.bytes_per_pixel_section(block_bounds);
