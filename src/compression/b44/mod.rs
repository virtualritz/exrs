...
pub fn decompress(
    ...
    // Note: expected_byte_size is now correctly calculated accounting for subsampling
    // via channels.bytes_per_pixel_section(rectangle) in the caller
    ...
    for channel in channels.list.iter() {
        let channel = ChannelData {
            ...
            samples_per_pixel: 1,  // Already accounted for in subsampled_resolution
        };
...
pub fn compress(
    ...
    // For subsampled channels, allocate extra space to handle block alignment
    let mut b44_compressed = vec![0; std::cmp::max(2048, uncompressed_le.len() * 2)];
